...
func GetDataByFilter(asaltujuan string, keyword string, orderBy, orderDir string, skip int, limit int) ([]models.Finances, int, error) {
	...
	if asaltujuan == "" {
		res := database.Conn.Raw("select id, type, asaltujuan, jumlah, keterangan, tanggal from finances where type = 'Pemasukan'")
		res.Scan(&totalData)
		...
		res := database.Conn.Table("select id, type, asaltujuan, jumlah, keterangan, tanggal from finances where type = 'Pemasukan'")
		res.Scan(&totalData)
		res.Order(orderBy + " " + orderDir).Limit(limit).Offset(skip).Scan(&data)
		err := res.Error
		if err != nil {
